...
class ElectricityGame:
    ...
    def build_ui(self):
        self.canvas.create_text(55, 48, anchor="w", text="⚡ SÄHKÖSEIKKAILU", fill=WHITE,
                                font=("Arial", 25, "bold"))
        self.rules_button_box = self.canvas.create_rectangle(
            55, 69, 125, 88, fill="#17314f", outline="#4b6683", width=1,
            tags=("rules_action",),
        )
        self.rules_button_text = self.canvas.create_text(
            90, 78, text="SÄÄNNÖT", fill=WHITE,
            font=("Arial", 8, "bold"), tags=("rules_action",),
        )
        self.advice_button_box = self.canvas.create_rectangle(
            132, 69, 207, 88, fill="#17314f", outline="#4b6683", width=1,
            tags=("advice_action",),
        )
        self.advice_button_text = self.canvas.create_text(
            169, 78, text="NEUVOJA", fill=WHITE,
            font=("Arial", 8, "bold"), tags=("advice_action",),
        )
        ...
        # )
        self.notation_button_box = self.canvas.create_rectangle(
            214, 69, 302, 88, fill="#17314f", outline="#4b6683", width=1,
            tags=("notation_action",),
        )
        self.notation_button_text = self.canvas.create_text(
            258, 78, text="MERKINNÄT", fill=WHITE,
            font=("Arial", 8, "bold"), tags=("notation_action",),
        )
        self.canvas.tag_bind("rules_action", "<Button-1>",
                             lambda _event: self.show_information("rules"))
        self.canvas.tag_bind("advice_action", "<Button-1>",
                             lambda _event: self.show_information("advice"))
        # TESTIPAINIKE:
        # self.canvas.tag_bind("gift_action", "<Button-1>",
        #                      self.claim_test_gift)
        self.canvas.tag_bind("notation_action", "<Button-1>",
                             lambda _event: self.show_information("notation"))
        self.canvas.tag_bind("rules_action", "<Enter>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.rules_button_box, fill="#28547b"))
        self.canvas.tag_bind("rules_action", "<Leave>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.rules_button_box, fill="#17314f"))
        self.canvas.tag_bind("advice_action", "<Enter>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.advice_button_box, fill="#28547b"))
        self.canvas.tag_bind("advice_action", "<Leave>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.advice_button_box, fill="#17314f"))
        # TESTIPAINIKE:
        # self.canvas.tag_bind("gift_action", "<Enter>",
        #                      lambda _event: self.canvas.itemconfigure(
        #                          self.gift_button_box, fill="#28547b"))
        # self.canvas.tag_bind("gift_action", "<Leave>",
        #                      lambda _event: self.canvas.itemconfigure(
        #                          self.gift_button_box, fill="#17314f"))
        self.canvas.tag_bind("notation_action", "<Enter>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.notation_button_box, fill="#28547b"))
        self.canvas.tag_bind("notation_action", "<Leave>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.notation_button_box, fill="#17314f"))
        ...
        self.canvas.create_text(500, 48, anchor="w", text="TEHTÄVÄ:", fill=MUTED,
                                font=("Arial", 11, "bold"))
        # Edistymispainike tehdään Canvasille samasta syystä kuin tehtävävalitsin:
        # koko suorakulmio reagoi varmasti klikkaukseen eri käyttöjärjestelmissä.
        self.progress_button_box = self.canvas.create_rectangle(
            390, 33, 485, 63, fill="#e7eef7", outline="#9aa5b1", width=2,
            tags=("progress_action",),
        )
        self.progress_button_text = self.canvas.create_text(
            437, 48, text="EDISTYMINEN", fill="#111820",
            font=("Arial", 9, "bold"), tags=("progress_action",),
        )
        self.canvas.tag_bind("progress_action", "<Button-1>",
                             self.progress_action_clicked)
        self.canvas.tag_bind("progress_action", "<Enter>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.progress_button_box, fill="#ffffff"))
        self.canvas.tag_bind("progress_action", "<Leave>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.progress_button_box, fill="#e7eef7"))
        self.root.bind("<Control-p>", lambda _event: self.show_progress())
        # Tehtävävalitsin piirretään suoraan Canvasille. Näin sen osuma-alue ei
        # riipu macOS:n natiivin Button/Menubutton-widgetin tilasta tai kursorista.
        self.task_selector_box = self.canvas.create_rectangle(
            565, 33, 680, 63, fill=WHITE, outline="#9aa5b1", width=2,
            tags=("task_selector",),
        )
        self.task_selector_text = self.canvas.create_text(
            622, 48, text="Tehtävä 1  ▾", fill="#111820",
            font=("Arial", 11, "bold"), tags=("task_selector",),
        )
        self.task_dialog = None
        self.canvas.tag_bind("task_selector", "<Button-1>",
                             self.task_selector_clicked)
        self.canvas.tag_bind("task_selector", "<Enter>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.task_selector_box, fill="#e7eef7"))
        self.canvas.tag_bind("task_selector", "<Leave>",
                             lambda _event: self.canvas.itemconfigure(
                                 self.task_selector_box, fill=WHITE))
        ...
        self.canvas.create_text(712, 130, anchor="w", text="TEHTÄVÄ", fill=CYAN,
                                font=("Arial", 13, "bold"))
        # Admittanssia tarvitsevien tehtävien käsitteen avaava lisätietopainike.
        # Painike on Canvasilla, jotta koko osuma-alue toimii luotettavasti
        # myös macOS:ssä.
        self.concept_info_box = self.canvas.create_rectangle(
            828, 116, 922, 144, fill="#17314f", outline="#4b6683", width=1,
            state="hidden", tags=("concept_info_action",),
        )
        self.concept_info_text = self.canvas.create_text(
            875, 130, text="LISÄTIETOA", fill=WHITE,
            font=("Arial", 8, "bold"), state="hidden",
            tags=("concept_info_action",),
        )
        self.canvas.tag_bind("concept_info_action", "<Button-1>",
                             self.show_concept_information)
        self.canvas.tag_bind(
            "concept_info_action", "<Enter>",
            lambda _event: self.canvas.itemconfigure(
                self.concept_info_box, fill="#28547b"),
        )
        self.canvas.tag_bind(
            "concept_info_action", "<Leave>",
            lambda _event: self.canvas.itemconfigure(
                self.concept_info_box, fill="#17314f"),
        )
        ...
        self.entry_window = self.canvas.create_window(712, 385, anchor="w", width=150,
                                                      height=48, window=self.entry)
        ...
        self.canvas.create_window(712, 455, anchor="w", width=210, height=48, window=self.button)
        ...
        # Nämä kaksi toimintoa piirretään suoraan Canvasille. macOS:n natiivit
        # Tk-painikkeet menettivät satunnaisesti klikkauksia Canvas-ikkunoina.
        self.hint_button_enabled = True
        self.solution_button_enabled = True
        self.bonus_button_enabled = True
        self.bonus_button_box = self.canvas.create_rectangle(
            712, 535, 782, 562, fill="#e7eef7", outline="#9aa5b1", width=2,
            tags=("bonus_action",),
        )
        self.bonus_button_text = self.canvas.create_text(
            747, 548, text=f"KYSYMYS +{BONUS_REWARD} P", fill="#111820",
            font=("Arial", 7, "bold"), tags=("bonus_action",),
        )
        self.hint_button_box = self.canvas.create_rectangle(
            782, 535, 852, 562, fill="#e7eef7", outline="#9aa5b1", width=2,
            tags=("hint_action",),
        )
        self.hint_button_text = self.canvas.create_text(
            817, 548, text=f"VIHJE −{HINT_COST} P", fill="#111820",
            font=("Arial", 7, "bold"), tags=("hint_action",),
        )
        self.solution_button_box = self.canvas.create_rectangle(
            852, 535, 922, 562, fill="#e7eef7", outline="#9aa5b1", width=2,
            tags=("solution_action",),
        )
        self.solution_button_text = self.canvas.create_text(
            887, 548, text=f"VASTAUS −{SOLUTION_COST} P", fill="#111820",
            font=("Arial", 7, "bold"), tags=("solution_action",),
        )
        self.canvas.tag_bind("bonus_action", "<Button-1>",
                             self.bonus_action_clicked)
        self.canvas.tag_bind("hint_action", "<Button-1>",
                             self.hint_action_clicked)
        self.canvas.tag_bind("solution_action", "<Button-1>",
                             self.solution_action_clicked)
        self.canvas.tag_bind("hint_action", "<Enter>",
                             lambda _event: self.set_action_hover("hint", True))
        self.canvas.tag_bind("hint_action", "<Leave>",
                             lambda _event: self.set_action_hover("hint", False))
        self.canvas.tag_bind("solution_action", "<Enter>",
                             lambda _event: self.set_action_hover("solution", True))
        self.canvas.tag_bind("solution_action", "<Leave>",
                             lambda _event: self.set_action_hover("solution", False))
        self.canvas.tag_bind("bonus_action", "<Enter>",
                             lambda _event: self.set_action_hover("bonus", True))
        self.canvas.tag_bind("bonus_action", "<Leave>",
                             lambda _event: self.set_action_hover("bonus", False))
        ...
        self.save_button = tk.Button(self.root, text="TALLENNA", command=self.save_game,
                                     ...
                                     takefocus=True)
        self.canvas.create_window(712, 570, anchor="nw", width=100, height=30,
                                  window=self.save_button)
        self.restart_button = tk.Button(self.root, text="ALOITA ALUSTA",
                                        command=self.restart_game,
                                        font=("Arial", 9, "bold"), cursor="hand2",
                                        takefocus=True)
        self.canvas.create_window(822, 570, anchor="nw", width=100, height=30,
    ...
    def show_challenge(self):
        ...
        info_state = "normal" if template_number in info_templates else "hidden"
        self.canvas.itemconfigure(self.concept_info_box, state=info_state)
        self.canvas.itemconfigure(self.concept_info_text, state=info_state)
        if info_state == "normal":
            self.canvas.tag_raise("concept_info_action")
        self.running = True
        self.message = "Syötä vastaus ja tarkista."
        self.message_color = MUTED
        ...
        self.canvas.itemconfigure(self.entry_window,
                                  width=190 if multiple_answers else 150)
    ...
    def configure_action_button(self, kind, enabled=None, text=None):
        """Päivittää Canvas-toimintopainikkeen tekstin ja käytettävyyden."""
        if kind == "bonus":
            box, text_item = self.bonus_button_box, self.bonus_button_text
            if enabled is not None:
                self.bonus_button_enabled = enabled
            active = self.bonus_button_enabled
            tag = "bonus_action"
        elif kind == "hint":
            box, text_item = self.hint_button_box, self.hint_button_text
            if enabled is not None:
                self.hint_button_enabled = enabled
            active = self.hint_button_enabled
            tag = "hint_action"
        else:
            box, text_item = self.solution_button_box, self.solution_button_text
            if enabled is not None:
        ...
        self.canvas.itemconfigure(box, fill="#e7eef7" if active else "#65717f")
        self.canvas.itemconfigure(text_item,
                                  fill="#111820" if active else "#c5ccd4")
        # Pitää koko osuma-alueen mahdollisen pitkän tilaviestitekstin päällä.
        self.canvas.tag_raise(tag)

    def set_action_hover(self, kind, hovering):
        """Antaa välittömän visuaalisen palautteen koko osuma-alueella."""
        if kind == "bonus":
            box, active = self.bonus_button_box, self.bonus_button_enabled
        elif kind == "hint":
            box, active = self.hint_button_box, self.hint_button_enabled
        else:
            box, active = self.solution_button_box, self.solution_button_enabled
        if active:
            self.canvas.itemconfigure(box,
                                      fill="#ffffff" if hovering else "#e7eef7")
    ...
    def close_bonus_dialog(self):
        ...
        if self.bonus_dialog is not None and self.bonus_dialog.winfo_exists():
            try:
                self.bonus_dialog.grab_release()
            except tk.TclError:
                pass
    ...
    def show_concept_information(self, _event=None):
        ...
        if template_number in ELECTRICAL_QUANTITY_INFO_TEMPLATES:
            sections.append(("JÄNNITE- JA VIRTASUUREET", (
                ...
                "Napajännite on lähteen plus- ja miinusnavan välinen jännite. "
        ...
        if template_number in ADMITTANCE_INFO_TEMPLATES:
            sections.append(("KOMPLEKSINEN ADMITTANSSI", (
    ...
    def close_solution_dialog(self, regenerate=False):
        ...
        if should_regenerate:
            ...
            # numero ensin, jotta samasta tehtävätyypistä arvotaan uudet arvot.
            self.question_number = current_task - 1
            # Tehtävä oli jo avattuna ennen vastauksen ostamista. Uusien
            # arvojen arvontaa ei estetä, vaikka ostos alittaisi ryhmän
            # avaamiseen vaaditun pisterajan.
